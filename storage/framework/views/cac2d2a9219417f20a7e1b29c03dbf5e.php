...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.table-header','data' => ['title' => 'Gestion des Employés','subtitle' => 'Gérez et suivez toute votre équipe de maniére centralisée','class' => 'animate-fade-in-up animation-delay-100']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $component->withAttributes(['title' => 'Gestion des Employés','subtitle' => 'Gérez et suivez toute votre équipe de maniére centralisée','class' => 'animate-fade-in-up animation-delay-100']); ?>
             ...
             <?php $__env->slot('actions', null, []); ?> 
                <div class="flex gap-3">
                    <a href="<?php echo e(route('admin.employees.export', request()->only(['department_id', 'status', 'contract_type']))); ?>" class="inline-flex items-center px-4 py-2.5 bg-white text-gray-700 font-medium rounded-xl border border-gray-200 hover:bg-gray-50 transition-all shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                        </svg>
                        Exporter
                    </a>
                    <a href="<?php echo e(route('admin.employees.create')); ?>" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg shadow-blue-500/25">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.filter-bar','data' => ['hasActiveFilters' => request()->hasAny(['search', 'department_id', 'status', 'contract_type']),'class' => 'animate-fade-in-up animation-delay-200']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['hasActiveFilters' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(request()->hasAny(['search', 'department_id', 'status', 'contract_type'])),'class' => 'animate-fade-in-up animation-delay-200']); ?>
             <?php $__env->slot('filters', null, []); ?> 
                ...
                <div class="w-full sm:w-auto">
                    <select name="status" class="w-full sm:w-40 px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm bg-white">
                        ...
                        <option value="on_leave" <?php echo e(request('status') == 'on_leave' ? 'selected' : ''); ?>>En congé</option>
                ...
                <div class="w-full sm:w-auto">
                    <select name="contract_type" class="w-full sm:w-40 px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm bg-white">
                        ...
                        <option value="cdi" <?php echo e(request('contract_type') == 'cdi' ? 'selected' : ''); ?>>CDI</option>
                        <option value="cdd" <?php echo e(request('contract_type') == 'cdd' ? 'selected' : ''); ?>>CDD</option>
                        <option value="stage" <?php echo e(request('contract_type') == 'stage' ? 'selected' : ''); ?>>Stage</option>
                    </select>
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <?php if(request()->hasAny(['search', 'department_id', 'status', 'contract_type'])): ?>
                        <a href="<?php echo e(route('admin.employees.index')); ?>" class="px-4 py-2.5 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 transition-colors flex items-center">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.data-table','data' => ['class' => 'animate-fade-in-up animation-delay-300']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['class' => 'animate-fade-in-up animation-delay-300']); ?>
             ...
             <?php $__env->slot('body', null, []); ?> 
                <?php $__empty_1 = true; $__currentLoopData = $employees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-blue-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <input type="checkbox" value="<?php echo e($employee->id); ?>" x-model="selected" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <div class="flex-shrink-0 h-10 w-10 relative">
                                    <?php if($employee->avatar): ?>
                                        <img class="h-10 w-10 rounded-full object-cover ring-2 ring-white shadow-sm" src="<?php echo e(avatar_url($employee->avatar)); ?>" alt="<?php echo e($employee->name); ?>">
                                    <?php else: ?>
                                    ...
                                    <span class="absolute -bottom-0.5 -right-0.5 block h-3 w-3 rounded-full ring-2 ring-white
                                        <?php if($employee->presence_status === 'present'): ?> bg-green-500
                                        <?php elseif($employee->presence_status === 'on_leave'): ?> bg-amber-500
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="font-medium text-gray-900 <?php echo e(in_array($employee->status, ['suspended', 'terminated']) ? 'line-through opacity-60' : ''); ?>"><?php echo e($employee->name); ?></span>
                                    ...
                                    <div class="text-xs text-gray-500 font-mono"><?php echo e($employee->employee_id); ?></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <?php if($employee->department): ?>
                                <div class="text-sm text-gray-900"><?php echo e($employee->department->name); ?></div>
                            <?php else: ?>
                                <div class="text-sm text-gray-400">-</div>
                            <?php endif; ?>
                            <div class="text-xs text-gray-500"><?php echo e($employee->position->name ?? 'Aucun poste'); ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <?php
                                $badges = [
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border <?php echo e($badges[$employee->contract_type] ?? 'bg-gray-100 text-gray-700 border-gray-200'); ?>">
                                <?php echo e(strtoupper($employee->contract_type ?? 'N/A')); ?>

                            </span>
                            <div class="text-xs text-gray-400 mt-1">
                                <?php echo e($employee->hire_date ? 'Depuis ' . $employee->hire_date->format('M Y') : '-'); ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-1">
                                <?php if($employee->email): ?>
                                    <a href="mailto:<?php echo e($employee->email); ?>" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="<?php echo e($employee->email); ?>">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <?php endif; ?>
                                <button type="button" 
                                        @click="confirmDelete('<?php echo e(route('admin.employees.destroy', $employee)); ?>')"
...
<?php endif; ?>
<?php /**PATH D:\ManageX\resources\views/admin/employees/index.blade.php ENDPATH**/ ?>