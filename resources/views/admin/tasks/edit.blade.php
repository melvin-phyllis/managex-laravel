            ...
            <form action="{{ route('admin.tasks.update', $task) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="titre" id="titre" value="{{ old('titre', $task->titre) }}" required class="w-full rounded-lg border-gray-300 focus:border-[#2D5A4E] focus:ring-[#2D5A4E] @error('titre') border-[#2D5A4E] @enderror">
                        @error('titre')
                            <p class="mt-1 text-sm text-[#1B3C35]">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="4" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A4E] focus:ring-[#2D5A4E] @error('description') border-[#2D5A4E] @enderror">{{ old('description', $task->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-[#1B3C35]">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="priorite" id="priorite" required class="w-full rounded-lg border-gray-300 focus:border-[#2D5A4E] focus:ring-[#2D5A4E] @error('priorite') border-[#2D5A4E] @enderror">
                    ...
                    <div>
                        ...
                        <input type="date" name="date_debut" id="date_debut" value="{{ old('date_debut', $task->date_debut?->format('Y-m-d')) }}" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A4E] focus:ring-[#2D5A4E] @error('date_debut') border-[#2D5A4E] @enderror">
                        @error('date_debut')
                            <p class="mt-1 text-sm text-[#1B3C35]">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_fin" id="date_fin" value="{{ old('date_fin', $task->date_fin?->format('Y-m-d')) }}" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A4E] focus:ring-[#2D5A4E] @error('date_fin') border-[#2D5A4E] @enderror">
                        @error('date_fin')
                            <p class="mt-1 text-sm text-[#1B3C35]">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="flex-1 bg-gray-200 rounded-full h-3">
                                <div class="bg-[#1B3C35] h-3 rounded-full" style="width: {{ $task->progression }}%"></div>
                            </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-[#E8F0ED] flex items-center justify-center">
                                    <svg class="w-4 h-4 text-[#1B3C35]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.tasks.document.download', $doc) }}" class="p-1.5 text-[#1B3C35] hover:bg-[#F0F5F3] rounded-lg transition" title="Telecharger">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                </a>
                                <label class="flex items-center gap-1 text-xs text-[#1B3C35] cursor-pointer hover:bg-[#F0F5F3] rounded-lg p-1.5 transition">
                                    <input type="checkbox" name="delete_documents[]" value="{{ $doc->id }}" class="rounded border-gray-300 text-[#1B3C35] focus:ring-[#2D5A4E] w-3.5 h-3.5">
                                    Suppr.
                                </label>
                            </div>
                        </div>
