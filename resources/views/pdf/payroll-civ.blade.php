    ...
    <div class="payslip-container">
        ...
        <table class="header-table">
            <tr>
                ...
                <td style="width: 35%;" class="company-info">
                    <strong>{{ config('app.name', 'ManageX') }}</strong><br>
                    {{ $user->company->address ?? 'Abidjan, Côte d\'Ivoire' }}<br><br>
                    CNPS Employé : {{ $user->cnps_number ?? 'N/A' }}<br>
                    Bull. n°{{ str_pad($payroll->id, 4, '0', STR_PAD_LEFT) }} du {{ $payroll->created_at->format('d/m/y') }}
                </td>
            </tr>
        </table>
        ...
        <table class="employee-table">
            ...
            <tr>
                ...
                <td>{{ number_format($payroll->fiscal_parts ?? 1, 1, ',', '') }}</td>
                <td>{{ $user->category ?? 'M2' }}</td>
                <td>{{ strtoupper($user->position->name ?? $user->poste ?? '-') }}</td>
                <td>{{ $user->section ?? '-' }}</td>
                <td>{{ str_pad($payroll->mois, 2, '0', STR_PAD_LEFT) }}/{{ substr($payroll->annee, 2) }}</td>
            </tr>
        </table>
        ...
        <table class="main-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right">{{ number_format($contract->base_salary ?? $payroll->base_salary ?? 0, 0, ',', ' ') }}</td>
                    <td class="text-center">30</td>
                    <td></td>
                    <td class="text-right">{{ number_format($contract->base_salary ?? $payroll->base_salary ?? 0, 0, ',', ' ') }}</td>
                </tr>
                ...
                <!-- Primes imposables -->
                @if(($payroll->seniority_bonus ?? 0) > 0)
                <tr>
                    <td class="text-center">32</td>
                    <td>Prime d'Ancienneté</td>
                    <td class="text-right">{{ number_format($payroll->seniority_bonus, 0, ',', ' ') }}</td>
                    <td class="text-center">30</td>
                    <td></td>
                    <td class="text-right">{{ number_format($payroll->seniority_bonus, 0, ',', ' ') }}</td>
                </tr>
                ...
                @if(($payroll->housing_allowance ?? 0) > 0)
                <tr>
                    <td class="text-center">35</td>
                    <td>Indemnité de Logement</td>
                    <td class="text-right">{{ number_format($payroll->housing_allowance, 0, ',', ' ') }}</td>
                    <td class="text-center">30</td>
                    <td></td>
                    <td class="text-right">{{ number_format($payroll->housing_allowance, 0, ',', ' ') }}</td>
                </tr>
                ...
                <tr class="totals-row">
                    ...
                    <td></td>
                    <td></td>
                    <td class="text-right">{{ number_format($payroll->taxable_gross, 0, ',', ' ') }}</td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="text-right">{{ number_format($payroll->tax_is, 0, ',', ' ') }}</td>
                    <td></td>
                </tr>
                <tr>
                    <td></td>
                    ...
                    <td></td>
                    <td></td>
                    <td class="text-right">{{ number_format($payroll->tax_cn, 0, ',', ' ') }}</td>
                    <td></td>
                </tr>
                <tr>
                    <td></td>
                    ...
                    <td></td>
                    <td></td>
                    <td class="text-right">{{ number_format($payroll->tax_igr, 0, ',', ' ') }}</td>
                    <td></td>
                </tr>
                <tr>
                    ...
                    <td class="text-right">{{ number_format($payroll->cnps_employee, 0, ',', ' ') }}</td>
                    <td></td>
                </tr>
                ...
                <tr class="totals-row">
                    ...
                    <td></td>
                    <td></td>
                    <td class="text-right">{{ number_format($payroll->taxable_gross - $payroll->total_deductions, 0, ',', ' ') }}</td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan="2" class="text-right net-pay-cell" style="font-size: 16px;">{{ number_format($payroll->net_salary, 0, ',', ' ') }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="footer-table">
            ...
            <tr>
                <td class="footer-value">{{ $payroll->mois * 30 }}</td>
                <td class="footer-value">{{ number_format($payroll->taxable_gross * $payroll->mois, 0, ',', ' ') }}</td>
                <td class="footer-value">{{ number_format($payroll->tax_is * $payroll->mois, 0, ',', ' ') }}</td>
                <td class="footer-value">{{ number_format($payroll->tax_cn * $payroll->mois, 0, ',', ' ') }}</td>
                <td class="footer-value">{{ number_format($payroll->tax_igr * $payroll->mois, 0, ',', ' ') }}</td>
                <td class="footer-value">{{ number_format($payroll->cnps_employee * $payroll->mois, 0, ',', ' ') }}</td>
                <td class="footer-value">30</td>
                <td class="footer-value">{{ number_format($payroll->mois * 2.2, 1, ',', ' ') }}</td>
            </tr>
        </table>
