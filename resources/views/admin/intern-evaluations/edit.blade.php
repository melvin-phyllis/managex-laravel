<x-layouts.admin>
    <div class="space-y-6" x-data="evaluationForm()">
        ...
        <x-table-header title="Modifier l'évaluation" subtitle="{{ $evaluation->week_label }}">
            <x-slot:icon>
                <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg" style="background-color: #1B3C35; box-shadow: 0 10px 15px -3px rgba(27, 60, 53, 0.2);">
        ...
        <!-- Draft Warning -->
        <div class="rounded-2xl p-4 flex items-center gap-3" style="background-color: rgba(27, 60, 53, 0.1); border: 1px solid rgba(27, 60, 53, 0.2);">
            <x-icon name="alert-circle" class="w-5 h-5" style="color: #1B3C35;" />
            <p class="text-sm" style="color: #1B3C35;">
        ...
        <form action="{{ route('admin.intern-evaluations.update', $evaluation) }}" method="POST" class="space-y-6">
                ...
                @foreach($criteria as $key => $criterion)
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="text-2xl font-bold" style="color: #1B3C35;" x-text="scores.{{ $key }}.toFixed(1) + '/2.5'"></div>
                        </div>
                        ...
                        <div class="mb-4">
                            <input type="range" 
                                   ...
                                   class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" style="accent-color: #1B3C35;">
                        ...
                        <textarea name="{{ $key }}_comment" 
                                  rows="3" 
                                  class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-[#1B3C35] focus:border-[#1B3C35] resize-none"
            ...
            <!-- Total Score Preview -->
            <div class="rounded-2xl p-6 text-white" style="background-color: #1B3C35;">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="general_comment" 
                              rows="4" 
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-[#1B3C35] focus:border-[#1B3C35] resize-none">{{ old('general_comment', $evaluation->general_comment) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="objectives_next_week" 
                              rows="3" 
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-[#1B3C35] focus:border-[#1B3C35] resize-none">{{ old('objectives_next_week', $evaluation->objectives_next_week) }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" name="action" value="submit" class="px-6 py-2.5 text-white rounded-xl font-medium transition-colors" style="background-color: #1B3C35;">
