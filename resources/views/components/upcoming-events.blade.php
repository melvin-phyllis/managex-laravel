...
<div {{ $attributes->merge(['class' => 'bg-white rounded-xl shadow-sm border border-gray-100 animate-fade-in-up']) }}>
    ...
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        @if(count($events) > $maxItems)
            <span class="text-xs text-gray-400">{{ count($events) }} total</span>
        @endif
    </div>

    {{-- Events List --}}
    <div class="divide-y divide-gray-50">
        @forelse(array_slice($events, 0, $maxItems) as $index => $event)
            @php
               ...
               style="animation-delay: {{ $index * 50 }}ms">
                <div class="flex items-start space-x-4">
                    ...
                    <div class="flex flex-col items-center">
                        <div class="w-10 h-10 rounded-full {{ $typeConfig['bg'] }} {{ $typeConfig['text'] }} flex items-center justify-center">
                            {!! $typeConfig['icon'] !!}
                        </div>
                        @if($index < min(count($events), $maxItems) - 1)
                    ...
                    <div class="flex-1 min-w-0 pb-4">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-medium {{ $typeConfig['text'] }} {{ $typeConfig['bg'] }} px-2 py-0.5 rounded-full">
                                {{ $typeConfig['label'] }}
                            </span>
                            <span class="text-xs {{ $isUrgent ? 'text-red-600 font-medium' : 'text-gray-400' }}">
                                @if($isToday)
                                    Aujourd'hui
                                @elseif($isTomorrow)
                                    Demain
    ...
    @if(count($events) > $maxItems)
        <div class="px-6 py-3 border-t border-gray-100 text-center">
            <a href="#" class="text-sm text-blue-600 hover:text-blue-800 font-medium">
                Voir tous les événements ({{ count($events) }})
            </a>
        </div>
    @endif
</div>
