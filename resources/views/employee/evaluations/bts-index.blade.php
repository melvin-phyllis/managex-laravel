<x-layouts.employee>
    <div class="space-y-6">
        <!-- Header -->
        <x-table-header title="Mes évaluations BTS" subtitle="Suivi de ma fiche de stage (/20)" class="animate-fade-in-up">
            <x-slot:icon>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in-up animation-delay-200">
            ...
            @if($latestEvaluation)
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="text-center">
                        @php
                            $gradeInfo = $latestEvaluation->grade_info;
                            $scorePercent = ($latestEvaluation->total_score / 20) * 100;
                        @endphp
                        <div class="inline-flex items-center justify-center w-20 h-20 rounded-full
                            ...
                            @endif">
                            <span class="text-2xl font-bold">{{ number_format($latestEvaluation->total_score, 1) }}</span>
                        </div>
                        <p class="mt-2 font-medium text-gray-900">{{ $gradeInfo['label'] }}</p>
                        <p class="text-sm text-gray-500">
                            {{ $latestEvaluation->stage_start_date->format('d/m/Y') }} - {{ $latestEvaluation->stage_end_date->format('d/m/Y') }}
                        </p>
            ...
            @if($averages)
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Moyennes par critère</h3>
                    <div class="space-y-3">
                        @foreach($criteria as $key => $criterion)
                            <div>
                                <div class="flex justify-between text-sm mb-1">
                                    <span class="text-gray-600">{{ $criterion['label'] }}</span>
                                    <span class="font-medium">{{ $averages[$key] }}/{{ $criterion['max'] }}</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-violet-600 h-2 rounded-full" style="width: {{ ($averages[$key] / $criterion['max']) * 100 }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <div class="mt-4 pt-4 border-t border-gray-100 text-center">
                        ...
                        <p class="text-2xl font-bold text-violet-600">{{ $averages['total'] }}/20</p>
                    </div>
                </div>
            @endif
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in-up animation-delay-300">
            ...
            @if($evaluations->isEmpty())
                <div class="p-12 text-center">
                    ...
                    <p class="text-sm text-gray-400 mt-1">Votre tuteur effectuera votre évaluation BTS</p>
                </div>
            @else
                <div class="divide-y divide-gray-100">
                    @foreach($evaluations as $evaluation)
                        @php
                            ...
                            $sp = ($evaluation->total_score / 20) * 100;
                        @endphp
                        <a href="{{ route('employee.evaluations.show', $evaluation) }}" class="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg
                                    @if($sp >= 80) bg-green-100 text-green-600
                                    @elseif($sp >= 60) bg-blue-100 text-blue-600
                                    @elseif($sp >= 50) bg-yellow-100 text-yellow-600
                                    @elseif($sp >= 40) bg-orange-100 text-orange-600
                                    @else bg-red-100 text-red-600
                                    @endif">
                                    {{ $gradeInfo['letter'] }}
                                </div>
                                <div>
                                    <p class="font-medium text-gray-900">
                                        {{ $evaluation->stage_start_date->format('d/m/Y') }} - {{ $evaluation->stage_end_date->format('d/m/Y') }}
                                    </p>
                                    <p class="text-sm text-gray-500">Par {{ $evaluation->evaluator->name ?? 'Évaluateur' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="text-right">
                                    <p class="text-2xl font-bold text-gray-900">{{ number_format($evaluation->total_score, 1) }}/20</p>
                                    <p class="text-sm text-gray-500">{{ $gradeInfo['label'] }}</p>
                                </div>
