...
<?php $component->withAttributes([]); ?>
    <div class="space-y-6" x-data="payrollTable()">
            ...
            <div class="relative p-6 md:p-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <a href="<?php echo e(route('admin.payrolls.create')); ?>" class="px-4 py-2.5 bg-white font-semibold rounded-xl hover:bg-purple-50 transition-all shadow-lg flex items-center" style="color: #5680E9;">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 animate-fade-in-up animation-delay-100">
            <form action="<?php echo e(route('admin.payrolls.index')); ?>" method="GET" class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                ...
                <div>
                    ...
                    <select name="mois" id="mois" class="rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <?php for($i = 1; $i <= 12; $i++): ?>
                            <option value="<?php echo e($i); ?>" <?php echo e(request('mois') == $i ? 'selected' : ''); ?>><?php echo e(ucfirst(\Carbon\Carbon::create()->month($i)->translatedFormat('F'))); ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select name="annee" id="annee" class="rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <?php for($i = now()->year; $i >= now()->year - 5; $i--): ?>
                            <option value="<?php echo e($i); ?>" <?php echo e(request('annee') == $i ? 'selected' : ''); ?>><?php echo e($i); ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select name="statut" id="statut" class="rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <option value="paid" <?php echo e(request('statut') == 'paid' ? 'selected' : ''); ?>>Payé</option>
                            ...
                            <tr class="hover:bg-purple-50/50 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <?php if($payroll->user->avatar): ?>
                                            <img class="h-9 w-9 rounded-full object-cover ring-2 ring-white shadow-sm" src="<?php echo e(avatar_url($payroll->user->avatar)); ?>" alt="<?php echo e($payroll->user->name); ?>">
                                        <?php else: ?>
                                            <div class="h-9 w-9 rounded-full flex items-center justify-center ring-2 ring-white shadow-sm" style="background: linear-gradient(135deg, #5680E9, #84CEEB);">
                                                <span class="text-white font-bold text-xs"><?php echo e(strtoupper(substr($payroll->user->name, 0, 2))); ?></span>
                                            </div>
                                        <?php endif; ?>
                                        <div>
                                            <div class="text-sm font-medium text-gray-900"><?php echo e($payroll->user->name); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium" style="color: #5680E9;"><?php echo e($payroll->periode); ?></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900"><?php echo e($payroll->net_salary_formatted ?? $payroll->montant_formatted); ?></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <?php if($payroll->statut === 'paid'): ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <a href="<?php echo e(route('admin.payrolls.download', $payroll)); ?>" class="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Télécharger PDF">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                            </svg>
                                        </a>
                                        <?php if($payroll->statut === 'pending'): ?>
                                            <form action="<?php echo e(route('admin.payrolls.mark-paid', $payroll)); ?>" method="POST" class="inline">
                                                <?php echo csrf_field(); ?>
                                        ...
                                        <?php endif; ?>
                                        <a href="<?php echo e(route('admin.payrolls.edit', $payroll)); ?>" class="p-1.5 text-gray-500 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Modifier">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <button type="button" 
                                                @click="confirmDelete('<?php echo e(route('admin.payrolls.destroy', $payroll)); ?>')"
        ...
        <div x-show="showBulkModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form action="<?php echo e(route('admin.payrolls.bulk-generate')); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                                        ...
                                        <div class="mt-4 grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <select name="mois" id="bulk_mois" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" required>
                                                    <?php for($i = 1; $i <= 12; $i++): ?>
                                                        <option value="<?php echo e($i); ?>" <?php echo e(now()->month == $i ? 'selected' : ''); ?>><?php echo e(ucfirst(\Carbon\Carbon::create()->month($i)->translatedFormat('F'))); ?></option>
                                                    <?php endfor; ?>
                                                </select>
                                            </div>
                                            <div>
                                                ...
                                                <select name="annee" id="bulk_annee" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" required>
                                                    <?php for($i = now()->year; $i >= now()->year - 2; $i--): ?>
                                                        <option value="<?php echo e($i); ?>" <?php echo e(now()->year == $i ? 'selected' : ''); ?>><?php echo e($i); ?></option>
                                                    <?php endfor; ?>
                                                </select>
                                            </div>
                                        </div>
...
<?php endif; ?><?php /**PATH D:\ManageX\resources\views/admin/payrolls/index.blade.php ENDPATH**/ ?>