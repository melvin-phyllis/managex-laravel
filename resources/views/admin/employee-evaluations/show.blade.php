            ...
            <div class="flex gap-3">
                @if($employeeEvaluation->canBeEdited())
                    <a href="{{ route('admin.employee-evaluations.edit', $employeeEvaluation) }}" 
                       class="inline-flex items-center gap-2 px-4 py-2 bg-[#1B3C35] text-white rounded-lg hover:bg-[#163530] transition-colors">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center gap-4">
                        <div class="w-16 h-16 bg-gradient-to-br from-[#2D5A4E] to-[#2D5A4E] rounded-full flex items-center justify-center text-white font-bold text-xl">
                        ...
                        @foreach($criteria as $key => $criterion)
                            ...
                            <div class="p-4 bg-gray-50 rounded-lg">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <span class="font-bold {{ $percentage >= 75 ? 'text-green-600' : ($percentage >= 50 ? 'text-yellow-600' : 'text-[#1B3C35]') }}">
                                        {{ number_format($value, 1) }}/{{ $max }}
                                    </span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="h-2 rounded-full transition-all {{ $percentage >= 75 ? 'bg-green-500' : ($percentage >= 50 ? 'bg-yellow-500' : 'bg-[#2D5A4E]') }}" 
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-6">
                    ...
                    <div class="text-center p-6 bg-gradient-to-br from-[#F0F5F3] to-[#FBF7F0] rounded-xl mb-6">
                        ...
                        <div class="flex items-baseline justify-center gap-1">
                            <span class="text-4xl font-bold {{ $employeeEvaluation->total_score >= 4 ? 'text-green-600' : ($employeeEvaluation->total_score >= 2.5 ? 'text-yellow-600' : 'text-[#1B3C35]') }}">
                                {{ number_format($employeeEvaluation->total_score, 1) }}
                            </span>
                            <span class="text-xl text-gray-400">/5,5</span>
                        </div>
                        <div class="mt-3 w-full bg-gray-200 rounded-full h-2">
                            <div class="h-2 rounded-full {{ $employeeEvaluation->total_score >= 4 ? 'bg-green-500' : ($employeeEvaluation->total_score >= 2.5 ? 'bg-yellow-500' : 'bg-[#2D5A4E]') }}" 
