...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['chatRoute' => null, 'isAdmin' => false]));
...
foreach (array_filter((['chatRoute' => null, 'isAdmin' => false]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<div x-data="aiChatWidget()" class="fixed bottom-6 right-6 z-50" @keydown.escape.window="open = false">
    ...
    <div x-show="open"
        ...
        <div class="px-4 py-3 flex items-center justify-between flex-shrink-0 <?php echo e($isAdminUser ? 'bg-gradient-to-r from-violet-600 to-indigo-600' : 'bg-gradient-to-r from-emerald-600 to-teal-600'); ?>">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-white"><?php echo e($isAdminUser ? 'Assistant IA Admin' : 'Assistant RH'); ?></h3>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3" x-ref="messagesContainer">
            ...
            <template x-if="messages.length === 0">
                <div class="text-center py-8">
                    ...
                    <p class="text-sm font-medium text-gray-900">Bonjour <?php echo e(auth()->user()->name); ?> !</p>
            ...
            <template x-for="(msg, index) in messages" :key="index">
                <div :class="msg.role === 'user' ? 'flex justify-end' : 'flex justify-start'">
                    <div :class="msg.role === 'user'
                        ? '<?php echo e($isAdminUser ? "bg-gradient-to-br from-violet-500 to-indigo-600" : "bg-gradient-to-br from-emerald-500 to-teal-600"); ?> text-white rounded-2xl rounded-br-md'
        ...
        <div class="border-t border-gray-200 p-3 flex-shrink-0">
            <form @submit.prevent="sendMessage" class="flex items-center gap-2">
                <input x-model="input"
                       ...
                       maxlength="500"
                       :disabled="loading"
                       class="flex-1 text-sm border border-gray-300 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-2 <?php echo e($isAdminUser ? 'focus:ring-violet-500 focus:border-violet-500' : 'focus:ring-emerald-500 focus:border-emerald-500'); ?> disabled:opacity-50 disabled:bg-gray-50"
                       @keydown.enter.prevent="sendMessage">
                <button type="submit"
                        :disabled="loading || !input.trim()"
                        class="w-10 h-10 text-white rounded-xl flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed transition-all flex-shrink-0 <?php echo e($isAdminUser ? 'bg-gradient-to-r from-violet-500 to-indigo-600 hover:from-violet-600 hover:to-indigo-700' : 'bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700'); ?>">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <button @click="toggleChat"
            class="w-14 h-14 text-white rounded-full shadow-lg hover:shadow-xl hover:scale-105 flex items-center justify-center transition-all duration-200 group relative <?php echo e($isAdminUser ? 'bg-gradient-to-r from-violet-500 to-indigo-600' : 'bg-gradient-to-r from-emerald-500 to-teal-600'); ?>">
        
        <span x-show="!open" class="absolute inset-0 rounded-full <?php echo e($isAdminUser ? 'bg-violet-400' : 'bg-emerald-400'); ?> animate-ping opacity-20"></span>
    ...
    return {
        ...
        async sendMessage() {
            ...
            try {
                ...
                const response = await fetch(<?php echo json_encode($resolvedRoute, 15, 512) ?>, {
                    method: 'POST',
                    headers: {
        ...
        formatMarkdown(text) {
            ...
            html = html.replace(/\n/g, '<br>');
            // Bullet points
            html = html.replace(/^- (.+)/gm, '<span class="flex gap-1.5 items-start"><span class="<?php echo e($isAdminUser ? 'text-violet-500' : 'text-emerald-500'); ?> mt-0.5">&#8226;</span><span>$1</span></span>');
            return html;
        }
    }
}
</script>
<?php /**PATH D:\ManageX\resources\views\components\ai-chat-widget.blade.php ENDPATH**/ ?>