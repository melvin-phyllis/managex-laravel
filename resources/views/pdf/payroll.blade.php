...
<head>
    ...
    <title>Fiche de Paie - {{ $payroll->periode }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="document-title">
        BULLETIN DE PAIE
        <br>
        <span style="font-size: 14px; font-weight: normal;">{{ $payroll->periode }}</span>
    </div>
        ...
        <tr>
            ...
            <td style="width: 48%; vertical-align: top; padding-left: 10px;">
                <div style="padding: 15px; border: 1px solid #e5e7eb; border-radius: 5px;">
                    ...
                    <div style="margin-bottom: 5px;"><span style="font-weight: bold;">Nom:</span> {{ $payroll->user->name }}</div>
                    <div style="margin-bottom: 5px;"><span style="font-weight: bold;">Email:</span> {{ $payroll->user->email }}</div>
                    <div style="margin-bottom: 5px;"><span style="font-weight: bold;">Poste:</span> {{ $payroll->user->poste ?? 'Non défini' }}</div>
                    <div style="margin-bottom: 5px;"><span style="font-weight: bold;">Téléphone:</span> {{ $payroll->user->telephone ?? 'Non défini' }}</div>
                </div>
            </td>
        </tr>
    </table>
    ...
    <div class="total-section">
        <div class="total-row">
            ...
            <span class="total-amount">{{ $payroll->montant_formatted }}</span>
        </div>
        <div style="margin-top: 15px;">
            <span style="font-weight: bold;">Statut du paiement:</span>
            <span class="status-badge {{ $payroll->statut === 'paid' ? 'status-paid' : 'status-pending' }}">
                {{ $payroll->statut === 'paid' ? 'Payé' : 'En attente' }}
            </span>
        </div>
    </div>
