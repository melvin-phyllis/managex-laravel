<x-layouts.employee>
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('employee.global-documents.index') }}" class="p-2 text-gray-400 hover:text-gray-600 transition">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6">
                ...
                <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                    <div class="w-12 h-12 rounded-lg bg-emerald-100 flex items-center justify-center text-xl">
                        {{ $globalDocument->file_icon }}
                    </div>
                    <div class="flex-1">
                        <p class="font-medium text-gray-900">{{ $globalDocument->original_filename }}</p>
                        <p class="text-sm text-gray-500">{{ $globalDocument->file_size_formatted }} • Ajouté le {{ $globalDocument->created_at->format('d/m/Y') }}</p>
                    </div>
                    <a href="{{ route('employee.global-documents.download', $globalDocument) }}"
                ...
                @if($globalDocument->description)
                    <div class="mt-4 p-4 border border-gray-200 rounded-lg">
                        ...
                        <p class="text-gray-600">{{ $globalDocument->description }}</p>
                    </div>
                @endif
            </div>
            ...
            @if(!$isAcknowledged)
                <div class="px-6 py-4 bg-amber-50 border-t border-amber-200">
                    <form action="{{ route('employee.global-documents.acknowledge', $globalDocument) }}" method="POST">
                        @csrf
