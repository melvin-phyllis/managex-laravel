...
<body>
    ...
    <div class="filter-info">
        <span><strong>Période:</strong> {{ $filters['date_debut'] ?? 'Début' }} au {{ $filters['date_fin'] ?? 'Aujourd\'hui' }}</span>
        @if(isset($filters['employee']) && $filters['employee'])
            <span><strong>Employé:</strong> {{ $filters['employee_name'] ?? 'Tous' }}</span>
        @endif
        <span><strong>Généré le:</strong> {{ now()->format('d/m/Y à H:i') }}</span>
    </div>
    ...
    <table style="width: 100%; margin-bottom: 20px;">
        <tr>
            <td style="width: 25%; text-align: center; padding: 10px; border: 1px solid #e5e7eb; background-color: #f9fafb;">
                <div style="font-size: 20px; font-weight: bold; color: #2563eb;">{{ $stats['total_presences'] ?? 0 }}</div>
                <div style="font-size: 10px; color: #6b7280; text-transform: uppercase;">Total Présences</div>
            </td>
            <td style="width: 25%; text-align: center; padding: 10px; border: 1px solid #e5e7eb; background-color: #f9fafb;">
                <div style="font-size: 20px; font-weight: bold; color: #059669;">{{ $stats['total_employees'] ?? 0 }}</div>
                <div style="font-size: 10px; color: #6b7280; text-transform: uppercase;">Employés</div>
            </td>
            <td style="width: 25%; text-align: center; padding: 10px; border: 1px solid #e5e7eb; background-color: #f9fafb;">
                <div style="font-size: 20px; font-weight: bold; color: #7c3aed;">{{ $stats['avg_hours'] ?? '0h' }}</div>
                <div style="font-size: 10px; color: #6b7280; text-transform: uppercase;">Moyenne/Jour</div>
            </td>
            <td style="width: 25%; text-align: center; padding: 10px; border: 1px solid #e5e7eb; background-color: #f9fafb;">
                <div style="font-size: 20px; font-weight: bold; color: #ea580c;">{{ $stats['total_hours'] ?? '0h' }}</div>
    ...
    <div style="text-align: right; font-size: 10px; color: #6b7280;">
        Total: {{ $presences->count() }} enregistrement(s)
    </div>
    @endif
