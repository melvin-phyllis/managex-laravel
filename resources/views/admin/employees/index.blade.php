                    ...
                    <tr class="hover:bg-[#F0F5F3]/50 transition-colors group">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                @endif
                                {{-- Acces section Demandes de stage --}}
                                <form method="POST" action="{{ route('admin.employees.toggle-stage-requests-access', $employee) }}" class="inline">
                                    @csrf
                                    <button type="submit"
                                            class="p-1.5 rounded-lg transition-colors {{ $employee->can_access_stage_requests ? 'text-emerald-700 hover:bg-emerald-50' : 'text-gray-500 hover:bg-gray-100' }}"
                                            title="{{ $employee->can_access_stage_requests ? 'Retirer acces Demandes de stage' : 'Donner acces Demandes de stage' }}">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
