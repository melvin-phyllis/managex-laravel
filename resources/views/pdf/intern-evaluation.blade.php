...
<head>
    ...
    <title>Évaluation - {{ $evaluation->intern->name ?? 'Stagiaire' }} - {{ $evaluation->week_label }}</title>
    <style>
...
<body>
    ...
    <div class="document-title">
        ...
        <br>
        <span style="font-size: 14px; font-weight: normal;">{{ $evaluation->week_label }}</span>
    </div>
    ...
    <table class="info-table">
        <tr>
            <td>
                <div class="info-box">
                    <h3>STAGIAIRE</h3>
                    <div class="info-row"><span class="info-label">Nom:</span> {{ $intern->name }}</div>
                    <div class="info-row"><span class="info-label">Email:</span> {{ $intern->email }}</div>
                    <div class="info-row"><span class="info-label">Département:</span> {{ $intern->department->name ?? 'Non assigné' }}</div>
                </div>
            </td>
            <td>
                <div class="info-box">
                    <h3>TUTEUR</h3>
                    <div class="info-row"><span class="info-label">Nom:</span> {{ $tutor->name }}</div>
                    <div class="info-row"><span class="info-label">Email:</span> {{ $tutor->email }}</div>
                    <div class="info-row"><span class="info-label">Date d'évaluation:</span> {{ $evaluation->submitted_at?->format('d/m/Y') ?? 'N/A' }}</div>
                </div>
            </td>
        </tr>
    </table>
    ...
    <div class="score-summary">
        <span class="score-big">{{ $evaluation->total_score }}/10</span>
        <span class="grade-letter">Grade {{ $evaluation->grade_letter }}</span>
        <span class="grade-badge grade-{{ strtolower($evaluation->grade_letter) }}">
            @php
                $labels = ['A' => 'Excellent', 'B' => 'Bien', 'C' => 'Satisfaisant', 'D' => 'À améliorer', 'E' => 'Insuffisant'];
            @endphp
            {{ $labels[$evaluation->grade_letter] ?? 'N/A' }}
        </span>
    </div>
        ...
        <tbody>
            ...
            @foreach($criteria as $key => $info)
                ...
                <tr>
                    ...
                    <td>{{ $info['desc'] }}</td>
                    <td class="score-cell {{ $colorClass }}">{{ $score }}/2.5</td>
                    <td>
    ...
    @if($evaluation->objectives_next_week)
        <div class="objectives-section">
            ...
            <p>{{ $evaluation->objectives_next_week }}</p>
        </div>
    @endif
