<x-layouts.admin>
    <div class="space-y-6" x-data="employeeTable()">
        <!-- Header -->
        <x-table-header title="Gestion des Employés" subtitle="Gérez et suivez toute votre équipe de manière centralisée">
            <x-slot:icon>
            ...
            <x-slot:actions>
                <div class="flex gap-3">
                    <button class="inline-flex items-center px-4 py-2.5 bg-white text-gray-700 font-medium rounded-xl border border-gray-200 hover:bg-gray-50 transition-all shadow-sm">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow group">
                ...
                <div class="mt-4">
                    <div class="flex items-center justify-between text-xs mb-1">
                        ...
                        <span class="text-green-600 font-bold">{{ $stats['total'] > 0 ? round(($stats['present'] / $stats['total']) * 100) : 0 }}%</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                        <div class="bg-green-500 h-1.5 rounded-full transition-all duration-1000" style="width: {{ $stats['total'] > 0 ? round(($stats['present'] / $stats['total']) * 100) : 0 }}%"></div>
                    </div>
                </div>
            </div>
        ...
        <!-- Filter Bar -->
        <x-filter-bar :hasActiveFilters="request()->hasAny(['search', 'department_id', 'status', 'contract_type'])">
            <x-slot:filters>
                    ...
                    <tr class="hover:bg-blue-50/50 transition-colors group">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <div class="flex-shrink-0 h-10 w-10 relative">
                                    @if($employee->avatar)
                                        <img class="h-10 w-10 rounded-full object-cover ring-2 ring-white shadow-sm" src="{{ Storage::url($employee->avatar) }}" alt="{{ $employee->name }}">
                                    @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-1">
                                ...
                                @if($employee->telephone)
                                    <a href="tel:{{ $employee->telephone }}" class="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="{{ $employee->telephone }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <form action="{{ route('admin.employees.destroy', $employee) }}" method="POST" class="inline" onsubmit="return confirm('Confirmer la suppression ?')">
                                    @csrf
                                    @method('DELETE')
