        ...
        <x-table-header title="Gestion des Présences" subtitle="Suivi des pointages et heures travaillées" class="animate-fade-in-up">
            ...
            <x-slot:actions>
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.presences.export.pdf', request()->query()) }}" class="inline-flex items-center px-4 py-2.5 bg-[#C8A96E]/10 text-[#C8A96E] font-medium rounded-xl border border-[#C8A96E]/20 hover:bg-[#C8A96E]/20 transition-all shadow-sm text-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow animate-fade-in-up animation-delay-100">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg" style="background-image: linear-gradient(135deg, #2D5A4E, #1B3C35) !important; box-shadow: 0 10px 15px -3px rgba(45, 90, 78, 0.3) !important;">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow animate-fade-in-up animation-delay-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg" style="background-image: linear-gradient(135deg, #C8A96E, #1B3C35) !important; box-shadow: 0 10px 15px -3px rgba(200, 169, 110, 0.3) !important;">
                        <x-icon name="clock" class="w-6 h-6 text-white" />
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow animate-fade-in-up animation-delay-300">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg" style="background-image: linear-gradient(135deg, #D4BC8B, #3D7A6A) !important; box-shadow: 0 10px 15px -3px rgba(212, 188, 139, 0.3) !important;">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow animate-fade-in-up animation-delay-400">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg" style="background-image: linear-gradient(135deg, #1B3C35, #2D5A4E) !important; box-shadow: 0 10px 15px -3px rgba(27, 60, 53, 0.3) !important;">
        ...
        <x-filter-bar :hasActiveFilters="request()->hasAny(['employee_id', 'date_debut', 'date_fin'])" class="animate-fade-in-up animation-delay-200">
            <x-slot:filters>
                ...
                <div class="flex-1 min-w-[200px] relative">
                    ...
                    <select name="employee_id" class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#2D5A4E] focus:border-[#2D5A4E] text-sm bg-white appearance-none">
                ...
                <div class="flex items-center gap-2">
                    <div class="relative">
                        <input type="date" name="date_debut" value="{{ request('date_debut') }}" class="pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#2D5A4E] focus:border-[#2D5A4E] text-sm">
                        <x-icon name="calendar" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    </div>
                    <span class="text-gray-400">à</span>
                    <div class="relative">
                        <input type="date" name="date_fin" value="{{ request('date_fin') }}" class="pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#2D5A4E] focus:border-[#2D5A4E] text-sm">
                        <x-icon name="calendar" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    </div>
                </div>
                ...
                <!-- Buttons -->
                <div class="flex gap-2">
                    <button type="submit" class="px-6 py-2.5 bg-[#1B3C35] text-white font-medium rounded-xl hover:bg-[#163530] transition-colors shadow-lg shadow-[#2D5A4E]/25 flex items-center">
                    ...
                    <tr class="hover:bg-[#F0F5F3]/50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <div class="flex-shrink-0 h-10 w-10">
                                    ...
                                    @else
                                        <div class="h-10 w-10 rounded-full flex items-center justify-center ring-2 ring-white shadow-sm" style="background-image: linear-gradient(135deg, #2D5A4E, #1B3C35) !important;">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex flex-col gap-1">
                                <div class="flex items-center text-xs">
                                    ...
                                    <span class="font-medium text-[#2D5A4E] bg-[#2D5A4E]/10 px-1.5 py-0.5 rounded">{{ $presence->check_in->format('H:i') }}</span>
                                </div>
                                <div class="flex items-center text-xs">
                                    <span class="w-16 text-gray-500">Départ:</span>
                                    @if($presence->check_out)
                                        <span class="font-medium text-[#C8A96E] bg-[#C8A96E]/10 px-1.5 py-0.5 rounded">{{ $presence->check_out->format('H:i') }}</span>
                                    @else
                                        <span class="text-xs text-[#1B3C35] italic">En cours...</span>
                                    @endif
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            ...
                            @elseif(!$presence->check_out)
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-[#2D5A4E]/10 text-[#2D5A4E] animate-pulse">
                                    <span class="w-1.5 h-1.5 bg-[#2D5A4E] rounded-full"></span>
                                    En cours
                                </span>
                            @else
