<x-guest-layout>
    <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5" x-data="{ loading: false }" @submit="loading = true">
            ...
            <button type="submit" :disabled="loading"
                class="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg shadow-blue-500/25 transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-75 disabled:cursor-not-allowed disabled:transform-none">
    ...
    <p class="text-center text-sm text-gray-500 mt-6">
        &copy; {{ date('Y') }} ManageX. Tous droits réservés.
    </p>
